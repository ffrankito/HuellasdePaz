...
import { db } from '@/db'
import { clientes, mascotas, planesConfig } from '@/db/schema'
  ...
  return (
    <div style={{ padding: '40px 48px', background: '#f9fafb', minHeight: '100vh' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 24, fontWeight: 600, color: '#111827', margin: 0 }}>Nuevo plan</h1>
        <p style={{ fontSize: 14, color: '#6b7280', marginTop: 4, marginBottom: 0 }}>Registrá un nuevo plan de previsión</p>
      </div>
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid #f3f4f6', padding: '32px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', maxWidth: 680 }}>
        <NuevoPlanForm clientes={clientesData} mascotas={mascotasData} planesConfig={planesConfigData} />
      </div>
    </div>
  )
}