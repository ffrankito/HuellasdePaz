...
function ActivarForm() {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    setListo(true)
    // Resolve destination: token from URL or look up by auth session
    const destino = await (async () => {
      if (token) return `/portal/${token}`
      const res = await fetch('/api/portal/mi-token')
      if (res.ok) {
        const data = await res.json()
        return `/portal/${data.token}`
      }
      return '/portal/login'
    })()
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        background: 'linear-gradient(160deg, #1a2e1f 0%, #111827 60%)',
        ...
        <div style={{
          ...
          {listo ? (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: '0 0 10px', letterSpacing: '-0.01em' }}>
                ¡Todo listo{nombre ? `, ${nombre}` : ''}!
              </h2>
              <p style={{ fontSize: 14, color: '#6b7280', margin: 0, lineHeight: 1.6 }}>
                Tu cuenta está activa. En un momento te llevamos a tu portal.
              </p>
            ...
            <>
              <div style={{ marginBottom: 24 }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: '0 0 6px', letterSpacing: '-0.01em' }}>
                  {nombre ? `Bienvenido/a, ${nombre}` : 'Activá tu cuenta'}
                </h2>
